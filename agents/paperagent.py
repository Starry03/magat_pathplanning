...
class PaperArchitectureAgent(BaseAgent):
    """
    #### Test 1
    Agent to load 'PaperArchitecture' model
    """
    ...
    def __init__(self, config):
        ...
        if not self.config.batch_numAgent:
            ...
            if self.config.bottleneckMode == "BottomNeck_only":
                from graphs.models.decentralplanner_bottleneck import (
                    DecentralPlannerNet,
                )

            elif self.config.bottleneckMode == "BottomNeck_skipConcat":
                from graphs.models.decentralplanner_bottleneck_SkipConcat import (
                    DecentralPlannerNet,
                )
        ...
        if self.config.test_num_processes == 0:
            ...
            print(
                "using multi threads for testing. Thread num: {}".format(
                    self.config.test_num_processes
                )
            )
        ...
        self.model = PaperArchitecture(self.config)
    ...
    def load_pretrained_checkpoint(self, epoch, lastest=True, best=False):
        ...
        try:
            ...
            checkpoint = torch.load(
                filename, map_location="cuda:{}".format(self.config.gpu_device)
            )
            ...
            self.logger.info(
                "Checkpoint loaded successfully from '{}' at (epoch {}) at (iteration {})\n".format(
                    self.config.checkpoint_dir_load,
                    checkpoint["epoch"],
            ...
            if self.config.train_TL:
                ...
                assert (
                    param_name_GFL != ""
                ), "you must specified the name of the parameters to be re-trained"
                for (
                    model_param_name,
                    model_param_value,
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.config.max_epoch + 1):
            ...
            self.logger.info(
                "Train {} on Epoch {}: Learning Rate: {}]".format(
                    self.config.exp_name, self.current_epoch, self.scheduler.get_lr()
                )
            )
            print(
                "Train {} on Epoch {} Learning Rate: {}".format(
    ...
    def train_one_epoch_noBatch(self):
        ...
        for batch_idx, (batch_input, batch_target, _, batch_GSO, _) in enumerate(
            ...
            if batch_idx % self.config.log_interval == 0:
                self.logger.info(
                    "Train {} on Epoch {}: [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
            ...
            if self.current_iteration % self.config.log_loss_interval == 0:
                ...
                self.summary_writer.add_scalar(
                    "iteration/loss", log_loss_avg, self.current_iteration
                )
                loss_record = []
    ...
    def test_step_noBatch(self):
        ...
        for batch_idx, (batch_input, batch_target, _, batch_GSO, _) in tqdm(
            ...
            if batch_idx % self.config.log_interval == 0:
                self.logger.info(
                    "ValidStep {} on Epoch {}: [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                        self.config.exp_name,
                        self.current_epoch,
                        batch_idx * len(inputGPU),
                        len(self.data_loader.validStep_loader.dataset),
                        100.0 * batch_idx / len(self.data_loader.validStep_loader),
                        loss_validStep.item(),
                    )
                )
        ...
        self.summary_writer.add_scalar(
            "epoch/loss_validStep", avg_loss, self.current_epoch
        )
    ...
    def train_one_epoch_Batch(self):
        ...
        for batch_idx, (batch_input, batch_target, _, batch_GSO, _) in tqdm(
            ...
            total=len(self.data_loader.train_loader),
        ):
    ...
    def test_step_Batch(self):
        ...
        log_loss_validStep = []
        for batch_idx, (batch_input, batch_target, _, batch_GSO, _) in tqdm(
            enumerate(self.data_loader.validStep_loader),
            total=len(self.data_loader.validStep_loader),
        ):

            inputGPU = batch_input.to(self.model.device)
            (B, N, C, W, H) = inputGPU.shape
            gsoGPU = batch_GSO.to(self.model.device)
            # gsoGPU = gsoGPU.unsqueeze(0)
            targetGPU = batch_target.to(self.model.device)
            batch_targetGPU = targetGPU.reshape(B * N, 5)
            self.optimizer.zero_grad()
            ...
            loss_validStep = loss_validStep + self.loss(
                predict, torch.max(batch_targetGPU, 1)[1]
            )
    ...
    def test_single(self, mode):
        ...
        self.summary_writer = self.recorder.summary(
            label, self.summary_writer, self.current_epoch
        )
    ...
    def test_multi(self, mode):
        ...
        with torch.no_grad():
            ...
            for input, target, makespan, _, tensor_map in dataloader:
                try:
                    task_queue.put(
                        (input, target, makespan, tensor_map, i, mode), block=True
                    )
    ...
    with torch.no_grad():
        while task_queue.qsize() > 0:
            ...
            try:
                ...
                thread_robot.setup(
                    input, load_target, makespanTarget, tensor_map, ID_dataset
                )
                ...
                for step in range(maxstep):
                    ...
                    if config.batch_numAgent:
                        ...
                        actionVec_predict = [
                            ts.detach().cpu() for ts in actionVec_predict
                        ]
                    ...
                    Time_cases_ForwardPass.append(time_ForwardPass)
                    allReachGoal, check_moveCollision, check_predictCollision = (
                        thread_robot.move(actionVec_predict, currentStep)
                    )
                ...
                store_GSO, store_communication_radius = (
                    thread_robot.count_GSO_communcationRadius(currentStep)
                )
                ...
                if allReachGoal and not check_CollisionHappenedinLoop:
                    ...
                    noReachGoalbyCollsionShielding = False
                    findOptimalSolution, compare_makespan, compare_flowtime = (
                        thread_robot.checkOptimality(True)
                    )
                ...
                if currentStep >= (maxstep):
                    findOptimalSolution, compare_makespan, compare_flowtime = (
                        thread_robot.checkOptimality(False)
                    )
