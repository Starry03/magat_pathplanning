...
class PaperArchitectureAgent(BaseAgent):
    ...
    def test_multi(self, mode):
        ...
        with torch.no_grad():
            ...
            for input, target, makespan, _, tensor_map in dataloader:
                try:
                    task_queue.put((input, target, makespan, tensor_map, i, mode), block=True)
...
def test_thread(thread_subid, thread_index, config, model, lock, task_queue,
    ...
    with torch.no_grad():
        while task_queue.qsize() > 0:
            try:
                input, load_target, makespanTarget, tensor_map, ID_dataset, mode = task_queue.get(block=False)
            ...
            try:
                ...
                thread_robot.setup(input, load_target, makespanTarget, tensor_map, ID_dataset)
